...
def get_stock_data(ticker):
    ...
    query = f"SELECT timestamp, open, close, high, low, volume FROM {table_name} ORDER BY timestamp ASC;"
...
updated_data = conn.execute(f"SELECT * FROM {ticker.lower()}_historical LIMIT 5;").fetchall()
print(updated_data)
